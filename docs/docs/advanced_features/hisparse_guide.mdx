...
For ordinary DSA, one logical KV entry covers one token. For DeepSeek-V4-Flash C4, one compressed KV entry covers four original tokens: the model's top-512 compressed-entry selection therefore covers 2048 tokens. Accordingly, `size=2048` selects 512 C4 entries; values not divisible by four round up so the requested token coverage is not reduced. The conversion happens once before allocating candidate/staging buffers, while selection and RESOLVE continue to use the Indexer's compressed logical index space.
...
The initial Random Prefetcher implementation is disabled with pipeline parallelism or speculative decoding and uses eager decode rather than CUDA graphs. It allocates one reusable staging buffer outside the configured resident `device_buffer_size`; its per-request physical capacity is the converted logical-entry count (512 C4 entries for 2048-token coverage). After the current sparse layer resolves, the next sparse layer's candidates are copied from host memory into staging on a low-footprint side stream while the current layer computes. Staging does not modify the resident table or LRU. Before the next layer resolves, its stream waits for staging; real top-k entries found there are promoted device-to-device through the normal resident-slot/LRU decision, and all other misses retain the normal host-to-device path. The staging candidates never become the attention page table and cannot replace the real Indexer top-k.
