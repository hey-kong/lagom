...
HiSparse with DSPARK currently runs TARGET_VERIFY eagerly. SGLang disables CUDA Graph capture for this combination because ragged request ownership can change between replays of the same token bucket; no additional command-line flag is required.
