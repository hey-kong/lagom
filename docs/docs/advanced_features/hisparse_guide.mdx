...
The Indexer produces a score-ordered Top-M, where M is the larger of the attention Top-k and the logical-entry count required by `size`. Attention still consumes only the first K positions; this prefetcher can consume the wider M-position ranking and warm those same logical positions in the next sparse layer. It never substitutes those positions for the next layer's own real Top-k: the next Indexer still runs normally, matching positions become resident hits, and all other real misses retain the normal host-to-device path. If `size` is omitted, it defaults to the runtime `effective_top_k` coverage.

For ordinary DSA, one logical KV entry covers one token. For DeepSeek-V4-Flash C4, one compressed KV entry covers four original tokens, so `size=2048` takes the preceding layer's first 512 compressed top-score positions. Values not divisible by four round up so the requested token coverage is not reduced. For example, with DeepSeek-V4-Flash K=512 compressed entries, `size=4096` requests M=max(512, 1024)=1024 compressed entries while attention continues to use only the first 512. The requested logical-entry count cannot exceed `device_buffer_size`. Extended Top-M (`size` greater than K coverage) is currently supported by the DeepSeek-V4 C4 path; generic DSA still requires `size` not to exceed K.
