...
With the default static DSPARK verify layout, HiSparse keeps decode CUDA Graph enabled and launches one request-parallel swap-in batch per verify step. For non-static ragged layouts, SGLang currently disables decode CUDA Graph because request ownership can change between replays of the same token bucket; no additional command-line flag is required. Prefill graph settings are unchanged.
