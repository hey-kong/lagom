...
Each DSPARK target verify is a HiSparse transaction. Before CUDA Graph capture, the target reserves whole-page C4 scratch for the maximum request count and verify width. Before each replay it updates the fixed device buffers that identify valid C4 logical locations and writer locations. After acceptance, accepted C4 KV is copied into the authoritative Host pool and fixed device buffer before sequence lengths are published; accepted and rejected logical mappings are then restored, while the fixed-address scratch remains reserved for the next replay. Current-verify attention selects transaction scratch unconditionally for matching C4 locations, so short-context and newest-slot fast paths cannot substitute stale KV for the model Indexer's Top-K.
