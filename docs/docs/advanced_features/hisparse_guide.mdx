...
<table style={{width: "100%", borderCollapse: "collapse", tableLayout: "fixed"}}>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>string / unset</td>
      <td>Optional HiSparse prefetch algorithm. When unset, preserves the existing model-specific automatic behavior. Currently supports <code>random</code>.</td>
    </tr>
    <tr>
      <td><code>prefetcher_config</code></td>
      <td>object / <code>{"{}"}</code></td>
      <td>Optional algorithm-specific configuration. For <code>random</code>, supports token-coverage <code>size</code> and <code>seed</code>.</td>
    </tr>
  </tbody>
</table>
...
### Random prefetcher (experimental)

Use the Random Prefetcher with its defaults:

```bash
--hisparse-config='{"top_k":2048,"device_buffer_size":6144,"host_to_device_ratio":10,"swap_in_block_size":960,"prefetcher":"random"}'
```

Or set the original-token coverage and an independent seed:

```bash
--hisparse-config='{"top_k":2048,"device_buffer_size":6144,"host_to_device_ratio":10,"swap_in_block_size":960,"prefetcher":"random","prefetcher_config":{"size":2048,"seed":0}}'
```

Random prefetch only warms the existing HiSparse GPU cache. The model Indexer still produces the real top-k, RESOLVE still loads any misses, and sparse attention still consumes only that real top-k. `size` specifies **original-token coverage**, not the number of compressed KV records. If it is omitted, the default covers the same number of original tokens as the runtime `effective_top_k`, including a model's `index_topk` override.

For ordinary DSA, one logical KV entry covers one token. For DeepSeek-V4-Flash C4, one compressed KV entry covers four original tokens: the model's top-512 compressed-entry selection therefore covers 2048 tokens. Accordingly, `size=2048` selects 512 C4 entries; values not divisible by four round up so the requested token coverage is not reduced. The conversion happens once before allocating candidate and miss-plan buffers, while selection and RESOLVE continue to use the Indexer's compressed logical index space.

When no `prefetcher` is specified, existing behavior is unchanged: eligible GLM-5.2 shared-index layers use automatic cross-layer prefetch, while DeepSeek-V4 does not prefetch. Legacy shared-index prefetch has priority over an explicitly configured Random Prefetcher; the two modes never operate on the same device buffer concurrently. `SGLANG_DISABLE_HISPARSE_PREFETCH=1` disables both modes.

The initial Random Prefetcher implementation is disabled with pipeline parallelism or speculative decoding and uses eager decode rather than CUDA graphs. It does **not** allocate an additional KV staging buffer. After the current sparse layer resolves, a plan-only RESOLVE applies the random candidates to the next sparse layer's existing resident table and LRU, recording which host records belong in which existing device-buffer slots without moving KV bytes. A low-footprint copy-only kernel then replays that plan directly into the next layer's device buffer on a side stream while the current layer computes. Before the next layer resolves, its stream waits for the copy event; random candidates that overlap the real top-k are ordinary resident hits, while all other real misses retain the normal host-to-device path. Random candidates never replace the real Indexer top-k or become the attention page table. This follows the same plan-then-copy structure as legacy shared-index prefetch, but Random must build a new plan for the target layer because its candidates are not the anchor's real top-k.
