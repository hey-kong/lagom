...
class LogitsMetadata:
    ...
    # When set, lm_head runs only on these output rows. DRAFT_EXTEND_V2 uses
    # EagleDraftExtendInput.select_index; OasisKV uses the normal/root rows
    # because draft logits are never sampled or committed.
    output_select_index: Optional[torch.Tensor] = None
    ...
    def from_forward_batch(cls, forward_batch: ForwardBatch):
        ...
        return cls(
            ...
            output_select_index=output_select_index,
        )
...
class LogitsProcessor(nn.Module):
    ...
    def _get_pruned_states(
        ...
        if (
            ...
            if logits_metadata.output_select_index is not None:
                # Only next_token_logits narrows to [bs, vocab]; the
                # FULL-capture hidden stays unpruned.
                pruned_states = hidden_states[logits_metadata.output_select_index]
            else:
